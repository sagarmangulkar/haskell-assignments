...
quadraticRoots a b c = ((b * (-1) + sqrt((b * b) - 4 * a * c))/(2 * a), (b * (-1) - sqrt((b * b) - 4 * a * c))/(2 * a))
...
multiples n = [0] ++ iterateFunction (\x->x+n) n
...
hailstones n = iterateFunction (\x-> if odd x == True then ((3 * x) + 1) else quot x 2) n
...
--hailstonesLen n = iterateFunction (\x-> if x == 1 then take 0 $ [x] else if odd x == True then ((3 * x) + 1) else quot x 2) n
hailstonesLen n = 1 + fromJust(1 `elemIndex` hailstones n)
-- Problem 6
...
sumAbsDiffs numberList = foldl (\b (x, y) -> abs(y - x) + b) 0 (zip numberList (tail numberList))
...
distances pt points = map (\(x, y) -> sqrt(((snd(pt) - y) * (snd(pt) - y)) + ((fst(pt) - x) * (fst(pt) - x)))) (points)
...
sumLengths pointsList = 
    foldl (\b (x, y) -> sqrt (((snd(x) - snd(y)) * (snd(x) - snd(y))) + ((fst(x) - fst(y)) * (fst(x) - fst(y))))+ b) 
    0 
    (zip pointsList (tail pointsList))
...
foldTree treeFn leafFn tree =
    case tree of
    Tree a b c -> treeFn (foldTree treeFn leafFn a)  b  (foldTree treeFn leafFn c)
    Leaf l -> leafFn l
...
catenateTreeLists tree =
    foldl (\a b -> a ++ b ) [] (flattenTree tree) 
